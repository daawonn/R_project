rm(list=ls())
cust_train <- read.csv("data_cust_2-3_train.csv", header=TRUE)
cust_test <- read.csv("data_cust_2-3_test.csv", header=TRUE)
...
write.csv(result_xgboost, "result_xgboost.csv", row.names = FALSE)
...
write.csv(result_svm, "result_svm.csv", row.names = FALSE)
